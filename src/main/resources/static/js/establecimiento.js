...
const handleOnClick = () => {
  ...
  $d.addEventListener("click", (e) => {
    if ($btnUpdate == e.target) {
      ...
      $inputTelephone.value = $inputTelephone.value.trim();
      $inputRuc.value = $inputRuc.value.trim();

      let isInvalid = false;

      if (
        !$inputName.value.match(
          "^(?=.{3,100}$)[A-ZÑÁÉÍÓÚ][A-ZÑÁÉÍÓÚa-zñáéíóú]+(?: [A-ZÑÁÉÍÓÚA-Za-zñáéíóú]+)*$"
        )
      ) {
      ...
      if (!$inputTelephone.value.match("^9[0-9]{8}$")) {
        if (!$inputTelephone.classList.contains("is-invalid"))
          $inputTelephone.classList.add("is-invalid");
        isInvalid = true;
      } else {
        if ($inputTelephone.classList.contains("is-invalid"))
      ...
      if (
        !$inputAddress.value.match(
          "^(?=.{3,100}$)[A-ZÑÁÉÍÓÚ][A-Za-zñáéíóú0-9.\\-]+(?: [A-Za-zñáéíóú0-9.\\-]+)*$"
        )
      ) {
        if (!$inputAddress.classList.contains("is-invalid"))
      ...
      if (!$inputRuc.value.match("^[1-9][0-9]{10,19}$")) {
        if (!$inputRuc.classList.contains("is-invalid"))
